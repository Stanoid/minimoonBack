      ...
      switch (query.func) {
        ...
        case "expireOrder":
          ...
          case "orderProccessor":
            ...
            const orderp = await strapi.db.query("api::order.order").findOne({
              select: ["*"],
              where: {
                id: oid,
              },
            ...
            if (orderp.status == "initiated") {
              if (orderp.payment_type == "online") {
                const sessionp = await stripe.checkout.sessions.retrieve(
                  orderp.session_id
                );
        ...
        case "getUserOrders":
          // user.id alreadey present, get userorders
          const uorders = await strapi.db.query("api::order.order").findMany({
        ...
        case "initPaymentSession":
          ...
          try {
            ...
            for (let i = 0; i < items.length; i++) {
              ...
              const ressub = await strapi.db
                ...
                .findOne({
                  ...
                  populate: ["varients", "varients.colors", "varients.sizes"],
                });
              ...
              for (let j = 0; j < ressub.varients.length; j++) {
                if (ressub.varients[j].id == items[i].id) {
                  ...
                  cartItems.push({
                    ...items[i],
                    color: ressub.varients[j].colors[0].id,
                    size: ressub.varients[j].sizes[0].id,
                  });
                }
              }
            ...
            if (payment_metod == 1) {
                ...
                shipping_options: [
                  {
                    shipping_rate_data: {
                      ...
                      fixed_amount: {
                        amount:
                          delivery_method == 1
                            ? pickup.home_price * 100
                            : pickup.pickup_price * 100,
                        currency: "dzd",
                      },
                      display_name:
                        delivery_method == 1
                          ? "توصيل إلى عنوان"
                          : "إستلام من نقطة توزيع",
                      delivery_estimate: {
                        minimum: {
                          unit: "business_day",
                          value: 5,
              ...
              return {
                url: `${process.env.CLIENT_URL}/payment?orderid=${prentry.id}`,
              };
            }
          } catch (e) {
        ...
        case "getOrders":
          ...
          const ressub = await strapi.db.query("api::order.order").findMany({
            select: ["*"],
          });
        ...
        case "getDeliveryOrders":
          ...
          const resdev = await strapi.db.query("api::order.order").findMany({
            select: ["*"],
            where: {
              status: "processed",
            },
          ...
          for (let i = 0; i < resdev.length; i++) {
            ...
            if (resdev[i].payment_type == "online") {
              ...
              ordob.city = resdev[i].pickup.name_ar;
              ordob.line1 =
                resdev[i].delivery_type == "pickup"
                  ? resdev[i].pickup.address_ar
                  : resdev[i].address;
              ordob.line2 = "";
        ...
        case "getAdminOrders":
          ...
          for (let i = 0; i < resdeva.length; i++) {
            ...
            if (resdeva[i].payment_type == "online") {
              ...
              ordob.city = resdeva[i].pickup.name_ar;
              ordob.line1 =
                resdeva[i].delivery_type == "pickup"
                  ? resdeva[i].pickup.address_ar
                  : resdeva[i].address;
              ordob.line2 = "";
