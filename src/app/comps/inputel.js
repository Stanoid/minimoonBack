...
<option  value={datum.id} key={index}>
 { props.iscats?datum.name_ar + " / ":datum.attributes.name_ar }
   { props.iscats?datum.catagory&&datum.catagory&&datum.catagory.name_ar :"" }
    ...
    <input
  value={props.value}
  onChange={(e) => props.outputfunc(e.target.value)}
  className="appearance-none block w-full bg-white border border-gray-200 rounded disabled:bg-gray-200 disabled:hover:cursor-not-allowed disabled:text-gray-900 py-3 px-3 leading-tight transition-colors text-sm text-right focus:outline-none focus:bg-white focus:border-moonsec-100/80"
  id="grid-last-name"
  ...
  placeholder={props.ispass ? "********" : props.label}
/>
